...
export default class DictionarySuggester extends FuzzySuggestModal<Definition> {
  ...
  constructor(plugin: WordNetPlugin) {
    ...
    setTimeout(async () => {
      ...
      if (
        await adapter.exists(this.plugin.manifest.dir + "/dict-MyDict.json")
      ) {
  ...
  query(term: string): any {
    ...
    if (this.customDict != null) {
      for (
        let i = 0;
        i < this.customDict.length && countOfFoundMatches < 30;
        i++
      ) {
  ...
  onChooseSuggestion(
    ...
    else new Notice(item.item.Term + " \n" + item.item.Definition, 10000);
  }
}
